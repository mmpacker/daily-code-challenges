...
function formatWithPadding (integer, pad, length) {
  return integer.toString().padStart(length, pad)
}
...
function mumble(string) {
  ...
  for( let i = 0; i < characters.length; i++) {
    ...
    result += characters[i].repeat(i + 1)
  }
  return result
}
...
function mapArray(array, funct) {
  ...
  array.forEach(function(element, index) {
    newArray.push(funct(element, index))
  })
  return newArray
}
...
function reduceArray(arr, funct, accum) {
  let acc = accum
  arr.forEach(function(element, index) {
    acc = funct(acc, element, index) 
  })
  return acc
}
